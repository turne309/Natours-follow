...
const createSendToken = (user, statusCode, res) => {
  ...
  // Sending the JWT inside the user's cookie. This makes it impossible for attackers to get ahold of the JWT
  if (process.env.NODE_ENV === "production") cookieOptions.secure = true;
  res.cookie("jwt", token, cookieOptions);

  // Removing password from the JSON output
...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  const testID = await User.findById(req.body.test);
  console.log(`TEST user obj: ${testID}`);
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  user.password = req.body.password;
  user.passwordConfirm = req.body.passwordConfirm;
  // Cannot use update() or passwordConfirm validator in schema will not run
  await user.save(); // Want validators to run here
